...
def draw_detections(frame, frame_processor, detections, output_transform, args):
    size = frame.shape[:2]
    ...
    for roi, landmarks, identity in zip(*detections):
        ...
        if capture_requested:
            ...
            if identity.id != FaceIdentifier.UNKNOWN_ID:
                print(CaptureName)
                folder_path = "database/{}".format(CaptureName)
                jpg_files = [f for f in os.listdir(folder_path) if f.endswith(".jpg")]
                print("Số file JPG:", len(jpg_files))
                save_path = "/database/{}/{}.jpg".format(CaptureName, len(jpg_files))
                cv2.imwrite(save_path, face_crop)
                print('Face image saved to {}'.format(save_path))
            else:
                CaptureName = input("Enter your name: ")
                PathDir = "database/{}".format(CaptureName)
                os.makedirs(PathDir, exist_ok=True)
                save_path = "database/{}/0.jpg".format(CaptureName)
                cv2.imwrite(save_path, face_crop)
                print('Face image saved to {}'.format(save_path))
                frame_processor.faces_database = FacesDatabase(args.fg, frame_processor.face_identifier,
                                            frame_processor.landmarks_detector,
                                            frame_processor.face_detector if args.run_detector else None, args.no_show)
                frame_processor.face_identifier.set_faces_database(frame_processor.faces_database)
...
def main():
    ...
    while True:
        ...
        frame = draw_detections(frame, frame_processor, detections, output_transform, args)
